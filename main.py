...
def main():
    ...
    # 3. Consulta SQL con parámetros (%s)
    query = """
    SELECT 
        rc.name as "Compañía",
    ...
    INNER JOIN account_fiscal_position afp ON afp.id = a.fiscal_position 
    INNER JOIN res_company rc ON rc.id = a.company_id
    INNER JOIN res_partner rp ON rp.id = a.partner_id
    INNER JOIN res_partner_address rpa ON rpa.id = a.address_invoice_id
    WHERE 
    ...
    try:
        with psycopg2.connect(**db_params) as conn:
            with conn.cursor() as cur:
                cur.execute(query, (fecha_inicio_str, fecha_fin_str))
    ...
    # 6. Eliminar el contenido existente desde la segunda fila
